...
export class CustomGridService {
  ...
  constructor(private _customGridDataService: CustomGridDataService) {
  ...
  refreshGrid() {
    ...
    switch (this.gridConfig.sourceType) {
      case SourceType.observable:
        ...
        if (this.sortConfig.length > 0) this.sortDataSource();
  ...
  pagingDataSource() {
    ...
    switch (this.gridConfig.sourceType) {
      case SourceType.observable:
        dataSourceLength = this.dataSourceComp.length;
        break;
      case SourceType.url:
    ...
    if (dataSourceLength > pageSize) {
      this.paginationConfig.pagesNumber = [
        ...Array(Math.ceil(dataSourceLength / pageSize)).keys(),
      ].map((i) => i + 1);
    } else {
  ...
  computeDataSource() {
    this.dataSourceComp = (this.dataSourceComp as Array<object>).slice(
  ...
  changePageSize(pageSize: number) {
    ...
    this.paginationConfig.currentPage = 1;
    if (this.gridConfig.sourceType == SourceType.url) this.computeGridRequest();
    this.refreshGrid();
  }

  computeGridRequest() {
    this.gridRequest.Skip =
      (this.paginationConfig.currentPage - 1) *
      this.gridConfig.gridRequest.Take;
    this.gridRequest.Take =
  ...
  nextPage() {
    let lastPage =
      this.paginationConfig.pagesNumber[
        this.paginationConfig.pagesNumber.length - 1
      ];
  ...
  filterDataSource() {
    this.filterConfig?.forEach((filter) => {
      this.dataSourceComp = (this.dataSourceComp as Array<Object>).filter(
        (f: any) => {
          if (filter.Field && filter.Value) {
            switch (typeof f[filter.Field]) {
              case 'string':
                return f[filter.Field]
                  .toLowerCase()
                  .includes((filter.Value as string).toLowerCase());
              case 'number':
                return f[filter.Field] === parseInt(filter.Value);
              default:
                return f[filter.Field] == filter.Value;
            }
          }
        }
      );
  ...
  addFilter(_filter: GridFilter) {
    debugger;
    let filterIndex = this.filterConfig?.findIndex(
      (f) => f.Field === _filter.Field
    );
    if (_filter?.Value) {
      if (filterIndex < 0) this.filterConfig?.push(_filter);
      else this.filterConfig[filterIndex] = _filter;
    } else {
  ...
  computeNextPrevEn() {
    ...
    this.paginationConfig.nextPageEnable =
  ...
  sortDataSource() {
    this.sortConfig.forEach((sort: GridSort) => {
      this.dataSourceComp = this.dataSourceComp.sort((a: any, b: any) => {
        if (sort.Field) {
  ...
  addSort(_sort: GridSort) {
    ...
    if (!sort) sorts?.push(_sort);
    else
      sorts.push(
        ...sorts.splice(
          sorts.findIndex((s) => _sort),
          1
        )
      );
